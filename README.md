...
| 越界（`<= x` 或 `>= y`，边界值不可猜） | 拒绝，提示「请输入 x+1 到 y-1 之间的整数」，**不消耗回合** |
| 非当前回合玩家提交 | 拒绝，提示「还没轮到你，请耐心等待」 |
| `guess == bomb` | **踩雷**，进入 Boom |
| `guess < bomb` | 安全，左边界抬升：`x = guess`，轮到下一位 |
| `guess > bomb` | 安全，右边界压低：`y = guess`，轮到下一位 |

**采用开区间 + 严格收缩**：边界值 `x` 与 `y` 代表已确认安全的范围，**不可作为猜测**（可猜范围为 `x+1` 到 `y-1`）；每次安全猜测都让区间**至少缩小 1**。由此保证两条不变量：

1. 炸弹始终严格落在 `(x, y)` 内（即 `x < bomb < y`）—— 界面提示的可猜范围（`x+1` 到 `y-1`）与实际合法范围完全一致，不会出现「按提示输入却被告知非法」；
...
| `game.left` / `right` | 当前开区间的边界（可猜范围为 `left+1` 到 `right-1`） |
...
```json
{ "type": "error", "code": "OUT_OF_RANGE", "message": "请输入 1 - 99 之间的整数" }
```
...
| `TestBoundaryShrink` | 边界抬升/压低、炸弹恒在开区间内、边界值不可猜、每回合至少缩 1、**200 次随机对局必然终止** |
